...
{
        Active::generate_dirs(promoting_men<Color, Board>(p.men(Color)), not_occupied(p), moves);
}

template<bool Color, typename Rules, typename Board>
int Driver<Color, Material::PAWN, Promotion, Rules, Board>::count(const Position<Board>& p)
{
        return Active::count_dirs(promoting_men<Color, Board>(p.men(Color)), not_occupied(p));
}

template<bool Color, typename Rules, typename Board>
bool Driver<Color, Material::PAWN, Promotion, Rules, Board>::detect(const Position<Board>& p)
{
        return Active::detect_dirs(promoting_men<Color, Board>(p.men(Color)), not_occupied(p));
}
