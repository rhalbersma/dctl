...
BOOST_AUTO_TEST_CASE(Board12x12)
{
        ...
        {
                auto const s = basic_state<canadian, board<rectangular<12, 12>>>::initial();
                traversal::test(drop_duplicates_gen, s, leafs);
        }

        {
                auto const s = basic_state<srilankan, board<rectangular<12, 12, false>>>::initial();
                traversal::test(drop_duplicates_gen, s, leafs);
        }
}
...
BOOST_AUTO_TEST_CASE(Board14x14)
{
        ...
        auto const s = basic_state<international, board<rectangular<14, 14>>>::initial();
