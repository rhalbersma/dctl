...
// The "Perft on board boards" thread on the FMJD forum (results until depth=9)
...
BOOST_AUTO_TEST_CASE(Ktar10x12)
{
        ...
        auto const s = state<international, board<ktar<10,12>>>::initial();
        traversal::test(drop_duplicates_gen, s, leafs);
}
...
BOOST_AUTO_TEST_CASE(Board12x10)
{
        ...
        auto const s = state<international, board<rectangular<12,10>>>::initial();
        traversal::test(drop_duplicates_gen, s, leafs);
}
