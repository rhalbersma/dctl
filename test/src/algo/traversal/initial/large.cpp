...
BOOST_AUTO_TEST_CASE(Board12x12)
{
        ...
        auto const p = state<canadian, board<rectangular<12, 12>>>::initial();
        traversal::test(drop_duplicates_gen, p, leafs);

        auto const q = state<srilankan, board<rectangular<12, 12, false>>>::initial();
        traversal::test(drop_duplicates_gen, q, leafs);
}

BOOST_AUTO_TEST_CASE(Board14x14)
{
        auto const p = state<international, board<rectangular<14, 14>>>::initial();
