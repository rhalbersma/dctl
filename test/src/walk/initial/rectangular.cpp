...
BOOST_AUTO_TEST_CASE(Ktar10x12)
{
        ...
        auto const p = Position<rules::International, board::Compact1012>::initial();
        test(p, unique_gen, leafs);

        // board with International initial moves and orthogonal capture support: does not fit into 64 bits
        auto const q = Position<rules::International, board::Ktar<10,12>>::initial();
...
BOOST_AUTO_TEST_CASE(Board12x10)
{
        ...
        auto const p = Position<rules::International, board::Compact1210>::initial();
        test(p, unique_gen, leafs);
