#include <dctl/rule_traits/optional/promotion_category.hpp>     // promotion_category_t, promotion_apres_fini_tag, promotion_en_passant_tag
#include <dctl/rules.hpp>                                       // Checkers, Czech, Frisian, International, Italian, Pool, Russian, Spanish, Thai
#include <boost/mpl/vector.hpp>                                 // vector
#include <boost/test/test_case_template.hpp>                    // BOOST_AUTO_TEST_CASE_TEMPLATE
#include <boost/test/unit_test.hpp>                             // BOOST_AUTO_TEST_SUITE, BOOST_AUTO_TEST_SUITE_END

namespace dctl {
namespace rules {

BOOST_AUTO_TEST_SUITE(RuleTraitsIsEnPassantPromotion)

using VariantsFalse = boost::mpl::vector
<
        checkers::Rules, czech::Rules, frisian::Rules, international::Rules, italian::Rules, pool::Rules, spanish::Rules, thai::Rules
>;

BOOST_AUTO_TEST_CASE_TEMPLATE(NotIsEnPassantPromotionV, T, VariantsFalse)
{
        static_assert(std::is_same<promotion_category_t<T>, promotion_sur_place_tag>::value, "");
}

using VariantsTrue = boost::mpl::vector
<
        russian::Rules
>;

BOOST_AUTO_TEST_CASE_TEMPLATE(IsEnPassantPromotionV, T, VariantsTrue)
{
        static_assert(std::is_same<promotion_category_t<T>, promotion_en_passant_tag>::value, "");
}

BOOST_AUTO_TEST_SUITE_END()

}       // namespace rules
}       // namespace dctl
