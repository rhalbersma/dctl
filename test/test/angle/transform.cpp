...
BOOST_AUTO_TEST_CASE(InverseIsInvolutionOnAllAngles)
{
        BOOST_CHECK(
                std::all_of(alpha, omega, [](auto const& i){
                        return group::IsInvolution()(std::bind(dctl::inverse, _1), Angle{i});
                })
        );
}
...
BOOST_AUTO_TEST_CASE(Rotate180IsInvolutionOnAllAngles)
{
        BOOST_CHECK(
                std::all_of(alpha, omega, [](auto const& i){
                        return group::IsInvolution()(std::bind(dctl::rotate, _1, 180_deg), Angle{i});
                })
        );
}
        ...
        BOOST_CHECK(
                std::all_of(alpha, omega, [](auto const& i){
                        return std::all_of(alpha, omega, [=](int const& j) {
                                return group::IsInvolution()(std::bind(dctl::mirror, _1, Angle{i}), Angle{j});
                        });
                })
        );
}
