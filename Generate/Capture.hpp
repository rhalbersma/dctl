...
Capture<Rules, Board>::Capture(const Position<Board>& p)
:
...
BitBoard Capture<Rules, Board>::promotions(BitBoard dest_sq) const
{
        return dest_sq & Board::PROMOTION[Color][0];
}

template<typename Rules, typename Board>
BitBoard Capture<Rules, Board>::captured_targets(void) const
{
...
template<typename Rules, typename Board>
BitBoard Capture<Rules, Board>::captured_kings(BitBoard captured_pieces) const
{
...
template<typename Rules, typename Board>
BitBoard Capture<Rules, Board>::captured_kings(BitBoard, Int2Type<false>) const
{
        return 0;
}
...
template<typename Rules, typename Board>
BitBoard Capture<Rules, Board>::captured_kings(BitBoard captured_pieces, Int2Type<true>) const
{
        return captured_pieces & king_targets_;
}

template<typename Rules, typename Board> template<bool Color>
bool Capture<Rules, Board>::is_promotion(BitBoard dest_sq) const
{
        return !Bit::is_zero(promotions<Color>(dest_sq));
}

template<typename Rules, typename Board>
bool Capture<Rules, Board>::is_promotion(void) const
{
        return current_.promotion;
}

template<typename Rules, typename Board>
bool Capture<Rules, Board>::current_greater_equal_best(void) const
{
        return greater_equal<Rules>()(current_, best_);
}

template<typename Rules, typename Board>
bool Capture<Rules, Board>::current_not_equal_to_best(void) const
{
        return not_equal_to<Rules>()(current_, best_);
}

template<typename Rules, typename Board>
void Capture<Rules, Board>::toggle_king_targets(void)
{
        initial_targets_ = remaining_targets_ ^= king_targets_;
}

template<typename Rules, typename Board>
void Capture<Rules, Board>::toggle_with_king(void)
{
        current_.with_king ^= true;
}

template<typename Rules, typename Board>
void Capture<Rules, Board>::toggle_promotion(void)
{
        current_.promotion ^= true;
}

template<typename Rules, typename Board>
void Capture<Rules, Board>::launch(BitBoard jump_sq)
{
        from_sq_ = jump_sq;
...
template<typename Rules, typename Board>
void Capture<Rules, Board>::finish(BitBoard jump_sq)
{
        not_occupied_ ^= jump_sq;
}
...
template<typename Rules, typename Board> FORCE_INLINE
void Capture<Rules, Board>::make(BitBoard target_sq)
{
...
template<typename Rules, typename Board>
void Capture<Rules, Board>::make(BitBoard target_sq, Int2Type<Variant::REMOVE_1>)
{
...
template<typename Rules, typename Board> FORCE_INLINE
void Capture<Rules, Board>::make(BitBoard target_sq, Int2Type<Variant::REMOVE_N>)
{
...
// tag dispatching on capture removal
template<typename Rules, typename Board> FORCE_INLINE
void Capture<Rules, Board>::undo(BitBoard target_sq)
{
...
// partial specialization for piece by piece removal during a capture sequence
template<typename Rules, typename Board> FORCE_INLINE
void Capture<Rules, Board>::undo(BitBoard target_sq, Int2Type<Variant::REMOVE_1>)
{
...
// partial specialization for complete removal after a capture sequence
template<typename Rules, typename Board> FORCE_INLINE
void Capture<Rules, Board>::undo(BitBoard target_sq, Int2Type<Variant::REMOVE_N>)
{
...
template<typename Rules, typename Board>
void Capture<Rules, Board>::improve_best(MoveList& move_list)
{
...
void Capture<Rules, Board>::add_man_capture(BitBoard dest_sq, MoveList& move_list)
{
        add_man_capture<Color>(dest_sq, move_list, Int2Type<Variant::is_AmbiguousManCapture<Rules>::VALUE>());
}

// partial specialization for man captures that are unambiguous
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Capture<Rules, Board>::add_man_capture(BitBoard dest_sq, MoveList& move_list, Int2Type<false>)
{
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Capture<Rules, Board>::add_man_capture(BitBoard dest_sq, MoveList& move_list, Int2Type<true>)
{
        const BitBoard captured_pieces = captured_targets();
...
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, MoveList& move_list)
{
        add_king_capture<Color, Index>(dest_sq, move_list, Int2Type<Variant::KingCaptureHalt<Rules>::VALUE>());
}

// partial specialization for kings that halt immediately if the final capture is a king, and slide through otherwise
template<typename Rules, typename Board> template<bool Color, size_t Index> FORCE_INLINE
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, MoveList& move_list, Int2Type<Variant::HALT_K>)
{
        if (king_targets_ & Pull<Board, Index>()(dest_sq))
                add_king_capture<Color, Index>(dest_sq, move_list, Int2Type<Variant::HALT_1>());
        else
                add_king_capture<Color, Index>(dest_sq, move_list, Int2Type<Variant::HALT_N>());
}

// partial specialization for kings that halt immediately after the final capture
template<typename Rules, typename Board> template<bool Color, size_t Index> FORCE_INLINE
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, MoveList& move_list, Int2Type<Variant::HALT_1>)
{
        const BitBoard captured_pieces = captured_targets();
        const bool ambiguous = !move_list.empty() && large<Rules>()(current_, captured_pieces);
...
template<typename Rules, typename Board> template<bool Color, size_t Index> FORCE_INLINE
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, MoveList& move_list, Int2Type<Variant::HALT_N>)
{
        assert(dest_sq & path());
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, BitBoard captured_pieces, BitBoard captured_kings, bool ambiguous, MoveList& move_list)
{
        add_king_capture<Color>(dest_sq, captured_pieces, captured_kings, move_list, Int2Type<Variant::PromotionCondition<Rules>::VALUE>());
        if (ambiguous && move_list.non_unique_back<Rules>())
                move_list.pop_back();
}
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, BitBoard captured_pieces, BitBoard captured_kings, MoveList& move_list, Int2Type<Variant::PROMOTE_BR>)
{
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Capture<Rules, Board>::add_king_capture(BitBoard dest_sq, BitBoard captured_pieces, BitBoard captured_kings, MoveList& move_list, Int2Type<Variant::PROMOTE_EP>)
{
        if (!is_promotion())
                move_list.push_back<Color, Rules>(from_sq_ ^ dest_sq,          captured_pieces, captured_kings);
        else
