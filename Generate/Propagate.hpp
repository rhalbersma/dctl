...
{
        return current_capture_.promotion;
}
...
template<typename Rules, typename Board>
void Propagate<Rules, Board>::toggle_king_targets(void)
{
...
{
        current_capture_.with_king ^= true;
}

template<typename Rules, typename Board>
void Propagate<Rules, Board>::toggle_promotion(void)
{
        current_capture_.promotion ^= true;
}
...
void Propagate<Rules, Board>::make(BitBoard target_sq, Int2Type<Variant::REMOVE_N>)
{
        ...
        increment<Rules>()(current_capture_, target_sq, king_targets_);
}
...
{
        decrement<Rules>()(current_capture_, target_sq, king_targets_);
        remaining_targets_ ^= target_sq;
}

template<typename Rules, typename Board>
void Propagate<Rules, Board>::improve_best(MoveList& move_list)
{
...
void Propagate<Rules, Board>::add_man_capture(MoveList& move_list, BitBoard dest_sq)
{
        add_man_capture<Color>(move_list, dest_sq, Int2Type<Variant::is_AmbiguousManCapture<Rules>::VALUE>());
}

// partial specialization for man captures that are unambiguous
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Propagate<Rules, Board>::add_man_capture(MoveList& move_list, BitBoard dest_sq, Int2Type<false>)
{
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Propagate<Rules, Board>::add_man_capture(MoveList& move_list, BitBoard dest_sq, Int2Type<true>)
{
        const BitBoard captured_pieces = captured_targets();
        const bool ambiguous = !move_list.empty() && large<Rules>()(current_capture_, captured_pieces);
        move_list.push_back<Color, Rules>(from_sq_ ^ dest_sq, promotions<Color>(dest_sq), captured_pieces, captured_kings(captured_pieces));
...
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq)
{
        add_king_capture<Color, Index>(move_list, dest_sq, Int2Type<Variant::KingCaptureHalt<Rules>::VALUE>());
}

// partial specialization for kings that halt immediately if the final capture is a king, and slide through otherwise
template<typename Rules, typename Board> template<bool Color, size_t Index> FORCE_INLINE
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq, Int2Type<Variant::HALT_K>)
{
        if (king_targets_ & Pull<Board, Index>()(dest_sq))
                add_king_capture<Color, Index>(move_list, dest_sq, Int2Type<Variant::HALT_1>());
        else
                add_king_capture<Color, Index>(move_list, dest_sq, Int2Type<Variant::HALT_N>());
}

// partial specialization for kings that halt immediately after the final capture
template<typename Rules, typename Board> template<bool Color, size_t Index> FORCE_INLINE
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq, Int2Type<Variant::HALT_1>)
{
        const BitBoard captured_pieces = captured_targets();
        const bool ambiguous = !move_list.empty() && large<Rules>()(current_capture_, captured_pieces);
        add_king_capture<Color>(move_list, dest_sq, captured_pieces, captured_pieces & king_targets_, ambiguous);
}

// partial specialization for kings that slide through after the final capture
template<typename Rules, typename Board> template<bool Color, size_t Index> FORCE_INLINE
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq, Int2Type<Variant::HALT_N>)
{
        assert(dest_sq & path());
        ...
        const bool ambiguous = !move_list.empty() && large<Rules>()(current_capture_, captured_pieces);
        do {
                add_king_capture<Color>(move_list, dest_sq, captured_pieces, captured_kings, ambiguous);
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq, BitBoard captured_pieces, BitBoard captured_kings, bool ambiguous)
{
        add_king_capture<Color>(move_list, dest_sq, captured_pieces, captured_kings, Int2Type<Variant::PromotionCondition<Rules>::VALUE>());
        if (ambiguous && move_list.non_unique_back<Rules>())
                move_list.pop_back();
}
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq, BitBoard captured_pieces, BitBoard captured_kings, Int2Type<Variant::PROMOTE_BR>)
{
...
template<typename Rules, typename Board> template<bool Color> FORCE_INLINE
void Propagate<Rules, Board>::add_king_capture(MoveList& move_list, BitBoard dest_sq, BitBoard captured_pieces, BitBoard captured_kings, Int2Type<Variant::PROMOTE_EP>)
{
        if (!is_promotion())
                move_list.push_back<Color, Rules>(from_sq_ ^ dest_sq,          captured_pieces, captured_kings);
        else
