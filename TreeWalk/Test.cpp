...
void TreeWalk::Test::International(void)
{
        ...
        Perft::perft<Variant::International>(i10, 11);
        ...
        Perft::perft<Variant::International>(random178, 9);
...
void TreeWalk::Test::Rectangular(void)
{
        ...
        Perft::perft<Variant::International>(iS, 9);

        std::cout << write_square_layout<Ktar11Board>()() << std::endl;
        std::cout << write_bit_layout<Ktar11Board>()() << std::endl;
        
        Position<Ktar11Board> iK11;
        Perft::perft<Variant::International>(iK11, 9);
                
        std::cout << write_square_layout<Ktar12Board>()() << std::endl;
        std::cout << write_bit_layout<Ktar12Board>()() << std::endl;
        
        Position<Ktar12Board> iK12;
        Perft::perft<Variant::International>(iK12, 9);
}

// The alternative game rules thread on the FMJD forum: http://laatste.info/bb3/viewtopic.php?f=53&t=2822
void TreeWalk::Test::ChessBoardVariants(void)
{
        Position<ChessBoard> i8;
        std::cout << write_square_layout<ChessBoard>()() << std::endl;
        
        Perft::perft<Variant::Brazilian>(i8, 13); // http://shashki.com/PNphpBB2-viewtopic-t-627-start-78.html
        Perft::perft<Variant::Pool>(i8, 13);      // http://shashki.com/PNphpBB2-viewtopic-t-627-start-57.html
        Perft::perft<Variant::Russian>(i8, 14);   // http://shashki.com/PNphpBB2-viewtopic-t-627-start-126.html
        Perft::perft<Variant::English>(i8, 14);   // http://shashki.com/PNphpBB2-viewtopic-t-627-start-126.html
        Perft::perft<Variant::Czech>(i8, 13);

        Position<RomanBoard> r8;
        std::cout << write_square_layout<RomanBoard>()() << std::endl;

        Perft::perft<Variant::Spanish>(r8, 13);
        Perft::perft<Variant::Italian>(r8, 13);
        ...
        Perft::perft<Variant::Italian>(ITA_Ed_Gilbert_2, 16);
        Perft::perft<Variant::Italian>(ITA_Ed_Gilbert_3, 12);
        
        Position<ThaiBoard> t8;
        Perft::perft<Variant::Thai>(t8, 11);      // Thai initial position with 2 rows of men
        Perft::perft<Variant::Thai>(i8, 13);      // Usual initial position with 3 rows of men
}

// Test positions from the official Italian rules: http://www.fid.it/regolamenti/2008/RegTec_CAPO_I.pdf
void TreeWalk::Test::ItalianRuleBook(void)
{
        ...
        for (size_t i = 0; i < 9; ++i)
                Divide::divide<Variant::Italian>(read_position_string<FEN_tag, RomanBoard>()(ITA_rules[i]), 1);
}
