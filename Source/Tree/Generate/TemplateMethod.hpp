...
{
        size_t num_moves = Driver<Color, Material, node::JUMPS, Rules, Board>::count(p);
        if (!num_moves)
                num_moves += Driver<Color, Material, node::MOVES, Rules, Board>::count(p);
        return num_moves;
}
...
{
        return Driver<Color, Material, node::JUMPS, Rules, Board>::count(p);
}

template<bool Color, int Material, typename Rules, typename Board>
size_t TemplateMethod<Color, Material, Rules, Board>::count_reverse(const node::Position<Board>& p) const
{
        return Driver<Color, Material, node::MOVES, Rules, Board>::count_reverse(p);
}

template<bool Color, int Material, typename Rules, typename Board>
size_t TemplateMethod<Color, Material, Rules, Board>::count_promotions(const node::Position<Board>& p) const
{
        return Driver<Color, Material, node::MOVES, Rules, Board>::count_promotions(p);
}

template<bool Color, int Material, typename Rules, typename Board>
size_t TemplateMethod<Color, Material, Rules, Board>::count_mobility(const node::Position<Board>& p) const
{
        return Driver<Color, Material, node::MOVES, Rules, Board>::count(p);
}
...
{ 
        return (
                Driver<Color, Material, node::MOVES, Rules, Board>::detect(p) || 
                Driver<Color, Material, node::JUMPS, Rules, Board>::detect(p)
        );
}
...
{ 
        return Driver<Color, Material, node::JUMPS, Rules, Board>::detect(p);
}

template<bool Color, int Material, typename Rules, typename Board> 
bool TemplateMethod<Color, Material, Rules, Board>::detect_reverse(const node::Position<Board>& p) const 
{
        return Driver<Color, Material, node::MOVES, Rules, Board>::detect_reverse(p);
}

template<bool Color, int Material, typename Rules, typename Board> 
bool TemplateMethod<Color, Material, Rules, Board>::detect_promotions(const node::Position<Board>& p) const 
{
        return Driver<Color, Material, node::MOVES, Rules, Board>::detect_promotions(p);
}
