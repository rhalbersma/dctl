...
private:
        ...
        constexpr auto row_e() const { return (width() +  ul_parity()) / 2; }
        constexpr auto row_o() const { return (width() + !ul_parity()) / 2; }
...
public:
        ...
        constexpr int size() const { return (width() * height()) / 2 + (width() * height() * ul_parity()) % 2; }
};
...
public:
        ...
        constexpr auto edge_lo() const { return left_down() + ul_parity(); }
