...
private:
        ...
        XSTD_PP_CONSTEXPR_INTRINSIC_MEM static auto file_table = []() {
                std::array<std::array<set_type, Board::width>, xstd::to_underlying_type(color::size)> table;
        ...
        XSTD_PP_CONSTEXPR_INTRINSIC_MEM static auto rank_table = []() {
                std::array<std::array<set_type, Board::height>, xstd::to_underlying_type(color::size)> table;
                for (auto&& c : { color::black, color::white }) {
...
private:
        XSTD_PP_CONSTEXPR_INTRINSIC_MEM static auto initial_table = []() {
                ...
                std::array<std::array<set_type, N>, xstd::to_underlying_type(color::size)> table;
                for (auto&& c : { color::black, color::white }) {
                        for (auto d = 0; d < N; ++d) {
