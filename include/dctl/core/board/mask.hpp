...
private:
        ...
        XSTD_PP_CONSTEXPR_INTRINSIC_MEM static auto file_table = []() {
                std::array<std::array<set_type, Board::width>, xstd::to_utype(color::size)> table;
                for (auto&& c : { color::black, color::white }) {
                        for (auto f = 0; f < Board::width; ++f) {
                                table[xstd::to_utype(c)][static_cast<std::size_t>(f)] =
        ...
        XSTD_PP_CONSTEXPR_INTRINSIC_MEM static auto rank_table = []() {
                std::array<std::array<set_type, Board::height>, xstd::to_utype(color::size)> table;
                for (auto&& c : { color::black, color::white }) {
                        for (auto r = 0; r < Board::height; ++r) {
                                table[xstd::to_utype(c)][static_cast<std::size_t>(r)] =
                                        squares_filter([=](int const sq) {
...
public:
        XSTD_PP_CONSTEXPR_INTRINSIC_FUN static auto file(color const c, int const f) // Throws: Nothing.
        {
                ...
                return file_table[xstd::to_utype(c)][static_cast<std::size_t>(f)];
        }
        ...
        XSTD_PP_CONSTEXPR_INTRINSIC_FUN static auto rank(color const c, int const r) // Throws: Nothing.
        {
                ...
                return rank_table[xstd::to_utype(c)][static_cast<std::size_t>(r)];
        }
...
private:
        XSTD_PP_CONSTEXPR_INTRINSIC_MEM static auto initial_table = []() {
                ...
                std::array<std::array<set_type, N>, xstd::to_utype(color::size)> table;
                for (auto&& c : { color::black, color::white }) {
                        for (auto d = 0; d < N; ++d) {
                                table[xstd::to_utype(c)][static_cast<std::size_t>(d)] = [=]() {
                                        set_type accum;
                                        for (auto r = 0; r < d; ++r) {
                                                accum ^= rank(c, r);
                                        }
...
public:
        XSTD_PP_CONSTEXPR_INTRINSIC_FUN static auto initial(color const c, int const separation) // Throws: Nothing.
        {
                ...
                return initial_table[xstd::to_utype(c)][static_cast<std::size_t>(d)];
        }
};
